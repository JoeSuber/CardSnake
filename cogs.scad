...
module mycog(wheel_height=15){
    ...
    translate([0,-(rad-adj), wheel_height/2])
        difference(){
            ...
            belt_angle(prf = tT5, rad=rad-adj, bwdth = wheel_height+0.1, angle=360,fn=256);
        }
}

module mybigcog(wheel_height=15){
    adj = 0.1;
    rad = 50.4/2 - (1.0*2);
    translate([0,-(rad-adj), wheel_height/2])
        difference(){
...
module belt_angle(prf = tT2_5, rad=25, bwdth = 6, angle=90,fn=128) {
	av=360/2/rad/3.141592654*tpitch[prf];
	bk=bk_thick[prf];
